...
class RoomDepartmentController{
    ...
    static async createRoom(req, res){
        ...
        */
        ...
        const {numberRoom, type,responsable} = req.body;
        console.log(req.body)
        try{ 
            const Department = await DepartmentTable.findOne({where:{name:type}})
            console.log(Department)
            if(Department){
                ...
                res.status(201).send(`Room ${numberRoom} created successfully!`)
            }
        }catch(error){
            if(error) console.log(error);
